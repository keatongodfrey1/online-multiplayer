 ...
 *
 * Plain data only (no classes/methods) so GameState can be deep-cloned with
 * structuredClone() and serialized for networking / save-games / undo. The
 * static board geometry (geometry.ts) is kept OUT of GameState precisely so
 * cloning stays cheap.
 */
