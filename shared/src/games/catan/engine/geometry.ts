 ...
 *
 * Ported from catan-clone/catan-engine/src/geometry.ts (see ./index.ts for the
 * port notes). One functional addition for this repo: spiralHexOrder(), used
 * by the official A-R spiral number-placement mode.
 *
...
import type { BoardState, PlayerId, PortType } from "./types.js";
...
/** HEX_DIRECTIONS indices in counter-clockwise angular order (0, 60, ... 300). */
const CCW_DIR_ORDER = [0, 5, 4, 3, 2, 1] as const;
...
function scaleCube(c: CubeCoord, k: number): CubeCoord {
...
// ----------------------------------------------------------------------------
// Ring spiral (the official "variable setup" lays the A-R number tokens in a
// spiral from a coastal corner inward). Integer cube-walk, no trig: for each
// ring from the outside in, start at the corner hex in direction
// CCW_DIR_ORDER[corner] and walk the ring counter-clockwise; consecutive rings
// start at the same angular corner, ending on the centre hex. Assumes a
// centred hexagonal board (the standard one); hexes outside the board are
// skipped, so partial/custom boards degrade gracefully.
// ----------------------------------------------------------------------------

export function spiralHexOrder(geo: BoardGeometry, startCorner = 0): HexId[] {
  const dist = (c: CubeCoord) => Math.max(Math.abs(c.x), Math.abs(c.y), Math.abs(c.z));
  const byKey = new Map<string, HexId>(geo.hexes.map((h) => [cubeKey(h.cube), h.id]));
  const maxR = Math.max(...geo.hexes.map((h) => dist(h.cube)));
  const corner = ((startCorner % 6) + 6) % 6;
  const out: HexId[] = [];
  for (let r = maxR; r >= 1; r--) {
    let cube = scaleCube(HEX_DIRECTIONS[CCW_DIR_ORDER[corner]!]!, r);
    for (let leg = 0; leg < 6; leg++) {
      // walking CCW, the first leg from a corner heads two CCW steps onward
      const d = HEX_DIRECTIONS[CCW_DIR_ORDER[(corner + leg + 2) % 6]!]!;
      for (let step = 0; step < r; step++) {
        const id = byKey.get(cubeKey(cube));
        if (id !== undefined) out.push(id);
        cube = addCube(cube, d);
      }
    }
  }
  const centre = byKey.get(cubeKey({ x: 0, y: 0, z: 0 }));
  if (centre !== undefined) out.push(centre);
  return out;
}
