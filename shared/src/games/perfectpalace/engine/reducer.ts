...
import { EMPTY_INVENTORY, EMPTY_STAKE, NOT_IN_DUNGEON, RESOURCE_OPTIONS } from './types.js'
import type { GameAction, ShopItem, BuildItem } from './actions.js'
import { advance, passedOrLandedOnStart, getSquare } from './board.js'
import { freshDeck, getCard, shuffle } from './cards.js'
import {
...
// ==================== Deterministic PRNG (server-owned) ====================
// The SERVER owns ALL randomness. Every deck shuffle and die roll is drawn from
// a single seeded PRNG whose state rides on GameState.rngState, so games are
// reproducible from a seed and survive save/resume. mulberry32 matches the
// other engines in this repo (catan/splendor) — changing it changes every
// shuffle/roll for a given seed, so don't touch it without re-baselining tests.
...
/**
 * Roll a d6 (1-6) from the state's seeded PRNG. Returns the value + next
 * rngState. Public so the ROOM and setup helper can generate server-owned dice
 * for the value-taking actions (initial rolls, duel rolls) and thread rngState
 * back into the engine before dispatch — clients never supply a die value.
 */
export function rollDieFrom(state: GameState): { value: number; rngState: number } {
  const { value, rngState } = advanceRng(state)
  return { value: Math.floor(value * 6) + 1, rngState }
}

// ==================== Initial state ====================

export function initialState(seed?: number): GameState {
  const seedValue = (seed ?? Math.floor(Math.random() * 0xffffffff)) >>> 0
  // Build the fresh deck and advance the PRNG past it (Catan's pattern), so the
  // first mid-game reshuffle does not repeat the opening deck order.
...
export function reducer(state: GameState, action: GameAction): GameState {
  switch (action.type) {
    ...
    // Turn: rolling. The SERVER rolls the die from the seeded PRNG; the client
    // only presses "Roll" (no value crosses the wire).
    case 'turn/rollDie': {
      const { value, rngState } = rollDieFrom(state)
      return commitRoll({ ...state, rngState }, value)
    }
    // Test-only seam: inject a scripted die value. The room NEVER constructs this
    // from a client message (it isn't whitelisted in sanitize.ts); only the
    // engine tests dispatch it directly.
...
// ==================== Safe wrapper for a networked server ====================
// The reducer has two failure modes against adversarial input: it silently
// NO-OPS by returning the SAME state reference on most illegal actions, and it
// THROWS on unknown player ids (findPlayer/findPlayerIndex). A networked room
// must crash on neither, and must know whether an action was actually applied
// (so it doesn't re-broadcast on a rejected message). tryReduce gives the room
// a typed result: ok:true only when the action produced a NEW state.

export type ReduceResult =
  | { ok: true; state: GameState }
  | { ok: false; error: string }

export function tryReduce(prev: GameState, action: GameAction): ReduceResult {
  try {
    const next = reducer(prev, action)
    // Reference equality: the reducer returns `prev` unchanged on a rejected
    // (no-op) action, so a same-reference result means "not applied".
    if (next === prev) return { ok: false, error: 'rejected (no-op)' }
